...
export default function WalkForm({ onSubmit, loading }) {
    ...
    function switchConstraint(type) {
        setConstraintType(type);
        // reset to sensible default when switching
        setValue(type === 'Duration' ? 30 : 2);
    }

    function handleSubmit() {
        const coords = location ?? { latitude: 54.5973, longitude: -5.9301 };
        onSubmit({ ...coords, constraintType, value: parseFloat(value) });
    }

    const label = constraintType === 'Duration'
        ? `Duration: ${value} mins`
        : `Distance: ${value} km`;
    ...
    return (
        <div className="walk-form">
            ...
            <div className="form-group">
                ...
                <button className="btn-location" onClick={useMyLocation}>
                    📍 {locationLabel ?? "Use my location (default: Belfast)"}
                </button>
            </div>
            ...
            <div className="form-group">
                ...
                <div className="toggle-group">
                    <button
                        className={`toggle-btn ${constraintType === 'Duration' ? 'active' : ''}`}
                        onClick={() => switchConstraint('Duration')}
                    >
                        Time
                    </button>
                    <button
                        className={`toggle-btn ${constraintType === 'Distance' ? 'active' : ''}`}
                        onClick={() => switchConstraint('Distance')}
                    >
                        Distance
                    </button>
                </div>
